...
// Lista de neveras
const fecthFridges = async () => {
  try {
    const id = props.establishmentId;
    console.log("Establecimiento ID:", id);
    const response = await axios.get(`/api/refrigerators/establishment/${id}`);
    fridges.value = response.data;
  } catch (error) {
    console.error("Error al obtener lista de neveras:", error.response ? error.response.data : error);
  }
};

onMounted(fecthFridges);
...
const updateNote = async () => {
  ...
  if (!note.value || note.value.trim() === "") {
    alert("La nota está vacia", "", "warning");
    return;
    }
  try {
    ...
    alert("Error al actualizar la nota", "", "success");
  }
};
